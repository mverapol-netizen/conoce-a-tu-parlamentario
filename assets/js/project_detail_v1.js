(() => {
  ...
  const renderVotes = (votes) => {
    ...
    return `<div class="project-votes">${votes.map((vote) => {
      ...
      const localDetail = vote.vote_id ? `votacion.html?id=${encodeURIComponent(vote.vote_id)}` : '';
      ...
      const explanation = voteType.includes('general')
        ...
        : voteType.includes('particular')
          ...
          : '';
      return `<article class="project-vote"><div class="project-vote-head"><div><h3>${escapeHtml(formatDate(vote.fecha))} · ${escapeHtml(vote.tipo_votacion_proyecto || vote.tipo_votacion || 'Votación')}</h3><span class="project-pill">${escapeHtml(vote.resultado || 'Resultado no informado')}</span></div><span class="project-pill">${escapeHtml(vote.quorum || 'Quórum no informado')}</span></div><p class="project-vote-object"><strong>Objeto registrado:</strong> ${escapeHtml(exactObject)}</p>${explanation ? `<p class="project-event-explain"><strong>Cómo leerla:</strong> ${escapeHtml(explanation)}</p>` : ''}<div class="project-vote-meta"><span class="project-pill">${escapeHtml(vote.tramite_constitucional || 'Trámite no informado')}</span>${vote.tramite_reglamentario ? `<span class="project-pill">${escapeHtml(vote.tramite_reglamentario)}</span>` : ''}${vote.sesion_numero ? `<span class="project-pill">Sesión ${escapeHtml(vote.sesion_numero)}</span>` : ''}</div><div class="project-counts"><span class="project-count">A favor ${asNumber(vote.total_si)}</span><span class="project-count">En contra ${asNumber(vote.total_no)}</span><span class="project-count">Abstención ${asNumber(vote.total_abstencion)}</span>${asNumber(vote.total_dispensado) ? `<span class="project-count">Dispensado ${asNumber(vote.total_dispensado)}</span>` : ''}</div><div class="project-vote-links">${localDetail ? `<a href="${escapeHtml(localDetail)}">Entender esta votación</a>` : ''}${source ? `<a href="${escapeHtml(source)}" target="_blank" rel="noopener noreferrer">Ver detalle nominal oficial</a>` : ''}</div></article>`;
    }).join('')}</div>`;
  };

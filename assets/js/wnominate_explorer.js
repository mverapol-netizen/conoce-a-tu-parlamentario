(() => {
  ...
  const els = {
    ...
    app: document.getElementById('wn-app'),
    svg: document.getElementById('wn-chart'),
    tooltip: document.getElementById('wn-tooltip'),
    card: document.getElementById('wn-member-card'),
    view1: document.getElementById('wn-view-1d'),
    view2: document.getElementById('wn-view-2d'),
    uncertainty: document.getElementById('wn-uncertainty'),
    context: document.getElementById('wn-context'),
    showAll: document.getElementById('wn-show-all'),
    download: document.getElementById('wn-download'),
    reset: document.getElementById('wn-reset'),
    resetZoom: document.getElementById('wn-reset-zoom'),
    commune: document.getElementById('wn-commune'),
    topic: document.getElementById('wn-topic'),
    region: document.getElementById('wn-region'),
    district: document.getElementById('wn-district'),
    partyList: document.getElementById('wn-party-list'),
    alignmentList: document.getElementById('wn-alignment-list'),
    memberSearch: document.getElementById('wn-member-search'),
    memberList: document.getElementById('wn-member-list'),
    chartEyebrow: document.getElementById('wn-chart-eyebrow'),
    chartTitle: document.getElementById('wn-chart-title'),
    chartDescription: document.getElementById('wn-chart-description'),
    chartFootnote: document.getElementById('wn-chart-footnote'),
    axisCaption: document.getElementById('wn-axis-caption'),
    topicBanner: document.getElementById('wn-topic-banner'),
    visibleCount: document.getElementById('wn-visible-count'),
    modelCount: document.getElementById('wn-model-count'),
    legend: document.getElementById('wn-legend'),
  };
  ...
  const alignmentLabel = (alignment) => political.blocks?.[alignment]?.label || ({oposicion:'Oposición',oficialismo:'Oficialismo',no_alineado:'No alineados'}[alignment] || 'Sin clasificación');
  ...
  const quantileSelection = (members, n = 20) => {
    ...
    const ids = new Set();
    for (let i = 0; i < n; i += 1) {
      const idx = Math.round(i * (ordered.length - 1) / (n - 1));
      ids.add(ordered[idx].id);
    }
    return ids;
  };

  const districtSelection = (members, districtId, n = 20) => {
    const eligible = members.filter((m) => Number(m.profile?.district) === Number(districtId));
    const selected = new Set(eligible.map((m) => m.id));
    if (!eligible.length) return quantileSelection(members, n);
    ...
    while (selected.size < Math.min(n, members.length) && candidates.length) {
      ...
      candidates.forEach((candidate, idx) => {
        const distance = Math.min(...eligible.map((anchor) => Math.abs(anchor.d1 - candidate.d1)));
        if (distance < bestDistance) { bestDistance = distance; bestIndex = idx; }
      });
      if (bestIndex < 0) break;
      selected.add(candidates[bestIndex].id);
  ...
  const buildFilters = () => {
    const regions = [...new Set(modelMembers.map((m) => m.profile?.region).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'es'));
    regions.forEach((region) => els.region.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(region)}">${escapeHtml(region)}</option>`));
    [...new Set(modelMembers.map((m) => Number(m.profile?.district)).filter(Number.isFinite))].sort((a,b)=>a-b).forEach((d) => els.district.insertAdjacentHTML('beforeend', `<option value="${d}">Distrito ${d}</option>`));
    ...
    communeRecords.sort((a,b)=>a.commune.localeCompare(b.commune,'es')).forEach((item) => {
      ...
      option.textContent = `${item.commune} · D${item.district}`;
    ...
    [...topicVotes.keys()].sort((a,b)=>a.localeCompare(b,'es')).forEach((topic) => {
      const count = topicVotes.get(topic).size;
      els.topic.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(topic)}">${escapeHtml(topic)} (${count})</option>`);
    });
  ...
  const renderMemberChecklist = () => {
    ...
    const visible = modelMembers
      .filter((m) => !term || normalize(m.name).includes(term))
  ...
  const visibleSets = () => {
    ...
    const selected = selectionForCurrentState(universe);
  ...
  const themeStats = (id) => {
    ...
    const key = `${state.topic}|${id}`;
    if (themeStatsCache.has(key)) return themeStatsCache.get(key);
    ...
    (memberVotes[String(id)] || []).forEach(([voteId, code]) => {
      ...
      if (code === 'A') stats.affirmative += 1;
      else if (code === 'E') stats.against += 1;
      else if (code === 'B') stats.abstention += 1;
      else if (code === 'N') stats.noVote += 1;
      else if (code === 'D') stats.excused += 1;
      stats.observed += 1;
    });
    themeStatsCache.set(key, stats);
    return stats;
  };

  const updateTopicBanner = () => {
    if (!state.topic) { els.topicBanner.hidden = true; els.topicBanner.innerHTML = ''; return; }
    const set = topicVotes.get(state.topic) || new Set();
    const meta = topicMeta.get(state.topic) || {};
    els.topicBanner.hidden = false;
    els.topicBanner.innerHTML = `<strong>${escapeHtml(state.topic)}</strong> · ${set.size} votaciones nominales clasificadas con este tema principal. Las coordenadas globales permanecen fijas; el filtro solo añade contexto sobre las decisiones registradas. <span>Taxonomía: ${escapeHtml(meta.taxonomy || 'institucional-híbrida')}</span>`;
  };
  ...
  const showTooltip = (event, member) => {
    ...
    const x = Math.min(event.clientX - wrap.left + 12, wrap.width - 292);
    const y = Math.max(8, event.clientY - wrap.top - 20);
  ...
  const domain1d = () => {
    const vals = modelMembers.map((m) => m.d1).filter(Number.isFinite);
    const min = Math.min(-1, ...vals);
    const max = Math.max(1, ...vals);
    const pad = (max - min) * .045;
    return [min - pad, max + pad];
  };

  const baseDomain2d = () => {
    const xs = modelMembers.map((m) => m.d1_2d).filter(Number.isFinite);
    const ys = modelMembers.map((m) => m.d2).filter(Number.isFinite);
    const xmin = Math.min(...xs), xmax = Math.max(...xs), ymin = Math.min(...ys), ymax = Math.max(...ys);
    return { xMin:xmin-.12, xMax:xmax+.12, yMin:ymin-.12, yMax:ymax+.12 };
  };

  const render1d = (sets) => {
    els.svg.innerHTML = '';
    els.svg.setAttribute('viewBox','0 0 1000 590');
    const W = 1000, H = 590, left = 65, right = 55, midY = 300;
    const [xmin,xmax] = domain1d();
    const xScale = (v) => left + (v-xmin)/(xmax-xmin)*(W-left-right);
    [-1,-.5,0,.5,1].forEach((tick) => {
      const x = xScale(tick);
      els.svg.appendChild(makeSvg('line',{x1:x,y1:70,x2:x,y2:515,class: tick===0 ? 'wn-zero-line':'wn-grid-line'}));
    ...
    selectedMembers.forEach((m,i)=> selectedY.set(m.id, midY + ((i%7)-3)*38));

    if (state.uncertainty) {
      selectedMembers.forEach((m) => {
        const b = bootstrapById.get(m.id);
        if (!b) return;
        const y = selectedY.get(m.id);
        const x1=xScale(b.q025), x2=xScale(b.q975);
    ...
    modelMembers.filter((m)=>sets.visible.has(m.id)).forEach((m) => {
      const selected = sets.selected.has(m.id);
      const y = selected ? selectedY.get(m.id) : midY;
      ...
      circle.addEventListener('mouseenter',(e)=>showTooltip(e,m));
      circle.addEventListener('mousemove',(e)=>showTooltip(e,m));
      circle.addEventListener('mouseleave',hideTooltip);
      circle.addEventListener('click',()=>openMemberCard(m));
      circle.addEventListener('keydown',(e)=>{ if(e.key==='Enter'||e.key===' '){e.preventDefault();openMemberCard(m);} });
      ...
      if (selected) {
        ...
        const anchor = xScale(m.d1)>W*.7?'end':(xScale(m.d1)<W*.3?'start':'middle');
        const tx = xScale(m.d1)+(anchor==='start'?10:anchor==='end'?-10:0);
        const ty = y-11;
        els.svg.appendChild(makeSvg('text',{x:tx,y:ty,'text-anchor':anchor,class:'wn-point-label'},m.name.split(' ').slice(0,2).join(' ')));
      }
    });
  };
  ...
  const render2d = (sets) => {
    els.svg.innerHTML = '';
    els.svg.setAttribute('viewBox','0 0 1000 590');
    const W=1000,H=590,left=65,right=45,top=35,bottom=55;
    const d = state.zoom2d || baseDomain2d();
    ...
    const yLabel=makeSvg('text',{x:15,y:(top+H-bottom)/2,'text-anchor':'middle',class:'wn-axis-text',transform:`rotate(-90 15 ${(top+H-bottom)/2})`},'Dimensión 2 · exploratoria');els.svg.appendChild(yLabel);

    modelMembers.filter((m)=>sets.visible.has(m.id)&&Number.isFinite(m.d2)).forEach((m)=>{
      const selected=sets.selected.has(m.id);
      const cx=xScale(m.d1_2d),cy=yScale(m.d2);
      ...
      circle.addEventListener('mouseenter',(e)=>showTooltip(e,m));circle.addEventListener('mousemove',(e)=>showTooltip(e,m));circle.addEventListener('mouseleave',hideTooltip);circle.addEventListener('click',()=>openMemberCard(m));circle.addEventListener('keydown',(e)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openMemberCard(m);}});els.svg.appendChild(circle);
      if(selected){if(state.activeId===m.id)els.svg.appendChild(makeSvg('circle',{cx,cy,r:11,class:'wn-ring'}));const rightSide=cx>W*.68;els.svg.appendChild(makeSvg('text',{x:cx+(rightSide?-9:9),y:cy-8,'text-anchor':rightSide?'end':'start',class:'wn-point-label'},m.name.split(' ').slice(0,2).join(' ')));}
    });
  };

  const render = () => {
    const sets = visibleSets();
    els.visibleCount.textContent = sets.visible.size;
    els.modelCount.textContent = modelMembers.length;
    updateTopicBanner();
    els.uncertainty.hidden = state.view !== '1d';
    els.resetZoom.hidden = state.view !== '2d';
    ...
    els.context.setAttribute('aria-pressed',String(state.showContext));
    els.uncertainty.setAttribute('aria-pressed',String(state.uncertainty));
    els.showAll.textContent = state.showAll ? 'Volver a selección' : 'Ver los 155';
    if(state.view==='1d'){
      els.chartEyebrow.textContent='W-NOMINATE · una dimensión';els.chartTitle.textContent='Principal patrón de votación';els.chartDescription.textContent='Ordena a los parlamentarios según el principal patrón de semejanza y diferencia observado en sus votos nominales de Sala.';els.chartFootnote.textContent='El signo del eje es una orientación matemática. Una posición más cercana a cero no equivale automáticamente a mayor moderación. El rango opcional proviene de bootstrap por proyectos.';els.axisCaption.innerHTML='<span>Valores negativos</span><span>Dimensión 1</span><span>Valores positivos</span>';render1d(sets);
    }else{
      els.chartEyebrow.textContent='W-NOMINATE · dos dimensiones';els.chartTitle.textContent='Mapa bidimensional exploratorio';els.chartDescription.textContent='La primera dimensión conserva el patrón principal; la segunda captura diferencias residuales y permanece deliberadamente sin una etiqueta política sustantiva.';els.chartFootnote.textContent='Usa la rueda o gesto de desplazamiento sobre el gráfico para acercar/alejar y arrastra para recorrer el plano. La segunda dimensión es exploratoria.';els.axisCaption.innerHTML='<span>Arrastra para mover</span><span>Rueda para zoom</span><span>D2 sin etiqueta sustantiva</span>';render2d(sets);
    }
  };

  const resetState = () => {
    state.view='1d';state.uncertainty=false;state.showContext=false;state.showAll=false;state.topic='';state.region='';state.district='';state.commune='';state.parties.clear();state.alignments.clear();state.members.clear();state.activeId=null;state.zoom2d=null;themeStatsCache.clear();
    els.commune.value='';els.topic.value='';els.region.value='';els.district.value='';els.memberSearch.value='';
    document.querySelectorAll('#wn-party-list input,#wn-alignment-list input').forEach((input)=>{input.checked=false;});
    renderMemberChecklist();els.card.hidden=true;render();
  };
  ...
  const bindEvents = () => {
    ...
    els.resetZoom.addEventListener('click',()=>{state.zoom2d=baseDomain2d();render();});
    ...
    els.svg.addEventListener('wheel',(e)=>{
      if(state.view!=='2d')return;e.preventDefault();const rect=els.svg.getBoundingClientRect();const d=state.zoom2d||baseDomain2d();const px=(e.clientX-rect.left)/rect.width;const py=(e.clientY-rect.top)/rect.height;const x=d.xMin+px*(d.xMax-d.xMin);const y=d.yMax-py*(d.yMax-d.yMin);const factor=e.deltaY<0?.84:1.19;const nx=(d.xMax-d.xMin)*factor,ny=(d.yMax-d.yMin)*factor;state.zoom2d={xMin:x-px*nx,xMax:x+(1-px)*nx,yMin:y-(1-py)*ny,yMax:y+py*ny};render();
    },{passive:false});
  ...
  const downloadPng = () => {
    const clone=els.svg.cloneNode(true);clone.setAttribute('xmlns',SVG_NS);const bg=document.createElementNS(SVG_NS,'rect');bg.setAttribute('x','0');bg.setAttribute('y','0');bg.setAttribute('width','100%');bg.setAttribute('height','100%');bg.setAttribute('fill','#ffffff');clone.insertBefore(bg,clone.firstChild);const xml=new XMLSerializer().serializeToString(clone);const blob=new Blob([xml],{type:'image/svg+xml;charset=utf-8'});const url=URL.createObjectURL(blob);const img=new Image();img.onload=()=>{const canvas=document.createElement('canvas');canvas.width=2000;canvas.height=1180;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.drawImage(img,0,0,canvas.width,canvas.height);URL.revokeObjectURL(url);const link=document.createElement('a');link.download=`wnominate-${state.view}-${state.topic?normalize(state.topic).replace(/\s+/g,'-'):'todos'}-2026.png`;link.href=canvas.toDataURL('image/png');link.click();};img.src=url;
  };

  const init = async () => {
    try {
      const [oneText,bootText,twoText,topicText,votesPayload]=await Promise.all([
      ...
      const one=parseCSV(oneText),boot=parseCSV(bootText),two=parseCSV(twoText),topics=parseCSV(topicText);
      const twoById=new Map(two.filter((r)=>r.spec_id==='raw_lop025_2d').map((r)=>[String(r.diputado_id),r]));
      modelMembers=one.map((r)=>{
        const id=String(r.diputado_id);const p=profileById.get(id)||{};const r2=twoById.get(id)||{};return{id,name:r.diputado_nombre||p.key||`ID ${id}`,d1:Number(r.dimension_1_raw),d1_2d:Number(r2.dimension_1_aligned),d2:Number(r2.dimension_2_aligned),binaryVotes:Number(r.binary_votes),profile:p};
      ...
      topics.forEach((r)=>{const topic=String(r.topic_primary||'').trim();const voteId=String(r.vote_id||'').trim();if(!topic||!voteId)return;if(!topicVotes.has(topic))topicVotes.set(topic,new Set());topicVotes.get(topic).add(voteId);if(!topicMeta.has(topic))topicMeta.set(topic,{taxonomy:r.taxonomy_version,method:r.topic_method});});
      memberVotes=votesPayload.members||{};
      initialSelection=quantileSelection(modelMembers,20);
      state.zoom2d=baseDomain2d();

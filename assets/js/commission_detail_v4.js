(() => {
  ...
  const init = async () => {
    ...
    root.innerHTML = '<div class="commission-loading">Cargando comisión, proyectos y actividad institucional…</div>';
    try {
      ...
      if (activityResponse.ok) {
        ...
        if (String(candidate.schema_version || '').startsWith('commission-activity-web-v0.4') && candidate.quality_gate?.passed === true) {
